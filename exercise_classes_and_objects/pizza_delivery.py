...
o	Otherwise, remove the given quantity of the ingredient and update the pizza price by removing\
...
class PizzaDelivery:
    ...
    def make_order(self):
        ...
        self.ordered = True
        formatted_ingredients = [f"{key}: {value}" for key, value in self.ingredients.items()]
        return f"You've ordered pizza {self.name} prepared with {', '.join(formatted_ingredients)} and the price will be {self.price}lv."
